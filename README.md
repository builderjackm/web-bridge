...
| `save_as_pdf` / `save-as-pdf` | `[--paper-format FORMAT] [--landscape] [--scale N] [--no-print-background] [--path PATH]` |
| `list_tabs` / `list-tabs` | 无 |
| `close_tab` / `close-tab` | 无 |
| `close_session` / `close-session` | 无 |
...
webbridge research mouse-click @e42
webbridge research key-type "literal text"
webbridge research send-keys "Mod+A"
...
webbridge research save-as-pdf --paper-format a4 --path /tmp/page.pdf
webbridge research list-tabs
```

以 `-` 开头的文本参数前面使用 `--`：

```bash
webbridge research key-type -- -literal
```
